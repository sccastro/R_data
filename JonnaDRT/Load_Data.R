...
ExtractDRT <- function(topdir) {
  ...
  for (i in 1:length(files)) {
    ...
    dat <- select(dat, time = drts_3, rt = drts_4, s2 = drts_5, s1 = drts_6, R = drts_7, response = drts_8)
    infile <- dat
...
#Combine Jonna DRT FILES
ExtractJDRT <- function(topdir) {
  filepath <- getwd()
  setwd(filepath)
  
  dirs <- list.dirs(path = topdir, full.names = TRUE) #get directories
  files <- list.files(path=dirs, pattern="*.txt", full.names=T, recursive=F) #get files
  subids1 <- regmatches(files, regexpr("\\d{3}", files)) #get subject ids
  subids <- substr(subids1, 2,3) #get just the id number
  condition <- regmatches(files, regexpr("[[:lower:]]{4}1|[[:lower:]]{4}2", files)) #get conditions
  ...
  for (i in 1:length(files)) {
    drts <- readLines(files[i])
    drts <- drts[drts != drts[1]]
    dat <- cSplit(data.frame(drts),"drts", ",")
    dat <- select(dat, trial = drts_02, time = drts_05, date = drts_08, rt = drts_10, r2g1 = drts_12, code = drts_13)
    infile <- dat
    infile$subid <- as.factor(subids[i])
    infile$condition <- as.factor(condition[i])
