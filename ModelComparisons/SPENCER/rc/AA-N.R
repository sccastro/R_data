...
# D=D[[i]]
# SR=c(pp="pm",pw="word",pn="nonword",nn="nonword",pfa="pm",
#                 wn="nonword",ww="word",nw="word")
# attr(SR,"pars") <- c("v")
...
#   if (length(CR)!=length(lR)) {
#     cat("Missing rcell/n")
#     tmp=D$D[D$D$C=="TRUE",]
#     print(sort(as.numeric(unique(tmp$rcell))))
#   }
  ...
  # get $ SR
  if ( is.null(SR) ) S <- NULL else {
    indx <- c(1:dim(D$D)[1])
    S <- indx
    first <- tapply(indx,D$D$lcell,min)
    for ( i in levels(D$D$rcell) ) {
      is.rcell <- D$D$rcell==i
      rcelli <- indx[is.rcell]
      firsti <- first[as.numeric(unique(as.character(D$D$lcell[is.rcell])))]
      SlRi <- as.character(D$D[firsti,D$SC[2]])
      names(firsti) <- SlRi      
      lookup <- as.character(D$D[firsti,D$SC[2]])
      names(lookup) <- as.character(D$D[firsti,D$R])
      for (j in rcelli) 
        S[j] <- firsti[ lookup[as.character(D$D[j,"lR"])] ]
    }
    attr(S,"pars") <- SR
  } 
  list(P=podr,D=dodr,N=Cpars,SR=S)
}
...
fitp2modelp <- function(p,pmap,D) {
  ...
  # Sort SR within cell
  if (!is.null(attr(D,"reorder")$SR)) for (i in attr(attr(D,"reorder")$SR,"pars")) 
    p[[i]] <- p[[i]][attr(D,"reorder")$SR]
...
#  print(i)     
      ...
      if (dim(dati)[1]>0) {
        pc <- p$pc[isin][1]         
        L <- c(L, pc*cp + (1-pc)*
          dfun(t=dati[[D$RT]],parlist=p[isin,],cv=dati[,D$cv,drop=F])) # likelihood
      }
    }
...
#     crct <- dat[dat$rcell==rcnams[i] & isrt,cnam]
    ...
    tmp <- rfun(ns=nsim,pi=p[isin,],tlohi=tlohi,maxsamp=maxsamp,sim.mult=sim.mult)
    sim[[i]] <- tmp$sims
