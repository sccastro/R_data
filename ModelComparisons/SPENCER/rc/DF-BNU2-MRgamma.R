# GAMMA NON-DECISION TIME, ANGLE EFFECTS SHAPE 
# SPECIFIC TO MR PROJECT ASSUMING $A ANGLE FACTOR
    ...
    if ( Ni>0 ) {
      ...
      if ( any(names(pi)=="st0") ) 
        if (pi$st0[1]>1e-3) # greater than 1ms 
          win[2,] <- win[2,] + rgamma(Ni,shape=pi$shape[1],scale=pi$st0[1])
      if ( remove.outliers ) {
...
# A=c(1,1); b=c(2,2); vC=c(2,1); vE=c(1,2); svC=c(.5,.5); svE=c(.5,.5); ter=c(.1,.1); st0=c(.1,.1)
# parlistC=list(A=A,b=b,v=vC,sv=svC,ter=ter)
# parlistE=list(A=A,b=b,v=vE,sv=svE,ter=ter)
# parlistCst=list(A=A,b=b,v=vC,sv=svC,ter=ter,st0=st0,shape=4)
# parlistEst=list(A=A,b=b,v=vE,sv=svE,ter=ter,st0=st0,shape=4)
# 
