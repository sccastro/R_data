...
start.model <- function(dati,pnams,Bounds) {
  ...
  p$AS <- 0.15 # slowdown in s at 180
  p$AI <- 0.1  # mirror intercept
  p
}
...
P2Mnatural <- function(p,pmap,D) {
  ...
  p$b <- p$A + p$B
  p$shape <- as.numeric(as.character(D$D$A))*p$AS/180
  if (any(names(p)=="AI"))
    p$shape[D$D$S=="mirror"] <- p$shape[D$D$S=="mirror"] + p$AI[D$D$S=="mirror"] 
  p
}
