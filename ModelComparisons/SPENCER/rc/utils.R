...
# topmodel=1; nesttol=.1; get.bma=F; subjects=NA; only.start=F
...
#    attr(DF[[mhname]]$MT,"dname") <- dname
  ...
  hmnames <- names(fitlist[[mname]]$MT[[1]])  

  if ( any(dir(dname)=="fitstatus.RData") ) { # hierarchical fit results   
    load(paste(dname,"fitstatus.RData",sep="/")) 
    if (is.numeric(topmodel)) topmodel <- hmnames[topmodel]
    if (!(topmodel %in% hmnames))
  ...
  tmp <- vector(mode="list",length=length(hmnames)*length(snams))
  names(tmp) <- hmnames
  out=list(pars=tmp,pars.names=tmp,pests=tmp,pests.names=tmp,stat=tmp,stat.names=tmp)
  cat(paste("  Getting results for participant(s):\n"))
  for (ss in 1:length(snams)) {
    cat(paste(snams[ss],""))
    for (i in 1:length(hmnames) ) {
      load(paste(paste(dname,snams[ss],"pmaps",hmnames[i],sep="/"),"RData",sep="."))
      names(attr(pmap,"best")) <- names(attr(pmap,"start"))
      save(pmap,file=paste(paste(dname,snams[ss],"pmaps",hmnames[i],sep="/"),"RData",sep="."))
      tmp=try(load(paste(paste(dname,snams[ss],"fits",hmnames[i],sep="/"),"RData",sep=".")),silent=TRUE)
      if (class(tmp)=="try-error") {
        stop(paste("Unable to load fit",paste(paste(dname,snams[ss],"fits",hmnames[i],sep="/"),"RData",sep=".")))
      }
      indx=i+(ss-1)*length(hmnames)
      npar=length(fit$par)
      pest=fit$pest
      noSE=any(is.na(fit$se))
      pname=NULL
      ptype=NULL
      for (j in names(pest)) {
        if (noSE) {
      ...
      out$pars[[indx]]=cbind(fitpar=fit$par,se=fit$se)      
      out$pests[[indx]]=pestval
      out$pars.names[[indx]]=cbind(rep(snams[ss],npar),
        hmname=rep(hmnames[i],npar))
      out$pests.names[[indx]]=cbind(rep(snams[ss],npest),
        hmname=rep(hmnames[i],npest),pname=pname,ptype=ptype)
      out$stat[[indx]]=c(fit$dev,fit$bic,fit$aic,
        length(fit$par),fit$convergence)
      out$stat.names[[indx]]=c(snams[ss],hmnames[i])
    }   
  }
  cat("\n")

  out$pars <- do.call("rbind",out$pars)
  out$pars.names <- do.call("rbind",out$pars.names)
  dimnames(out$pars.names)[[2]][1] <- "s"
  out$pests <- unlist(out$pests)
  names(out$pests) <- 
    unlist(lapply(strsplit(names(out$pests),".",fixed=T),function(x){
      paste(x[-1],collapse=".")}))
  out$pests.names <- do.call("rbind",out$pests.names)
  dimnames(out$pests.names)[[2]][1] <- "s"
  out$stat <- do.call("rbind",out$stat)
  dimnames(out$stat) <- list(NULL,c("dev","bic","aic","npar","convergence"))
  out$stat.names <- do.call("rbind",out$stat.names)
  dimnames(out$stat.names) <- list(NULL,c("s","hmname"))
  ...
  for (i in names(ok)) {
    ok[i] <- all( hmnames %in% fitlist[[mname]]$fits[["stats.names"]]
