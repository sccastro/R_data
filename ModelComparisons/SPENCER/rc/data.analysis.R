...
mneffects=function(df,elist,digits=3,err=FALSE,vars=F,save=FALSE,
  stat="mean") {
  dvnam=dim(df)[2]
...
#   S=NULL,R=NULL,F=NULL,RT=NULL,CV=NULL,SC=NULL,cv=NULL,
#   default.scores=NA,autoscore=NA,save.trials=NA,
#   precision.rt=.001,autoscore.list=NA) {
#   # alldat = data frame
...
#             cat("\n")
#           } else if ( (!is.list(autoscore) & !any(is.na(autoscore))) || 
#                       (is.list(autoscore) && !any(is.na(autoscore[[i]]))) ) {
#             cat(paste("Autoscoring",dim(Di$D[is.na(Di$D[[i]]),])[1],
#               "cells for participant",j,"\n\n"))
#             if ( !any(is.na(autoscore.list)) && !any(is.na(autoscore.list[[i]])) ) {
#               for (rlev in levels(Di$D[[autoscore[2]]])) {
#                 isin <- Di$D[[autoscore[2]]]==rlev
#                 Di$D[is.na(Di$D[[i]]) & isin,i] <- 
#                   Di$D[is.na(Di$D[[i]]) & isin,autoscore[1]] %in% autoscore.list[[rlev]]  
#               }  
#             } else Di$D[is.na(Di$D[[i]]),i] <- 
#               Di$D[is.na(Di$D[[i]]),autoscore[1]]==Di$D[is.na(Di$D[[i]]),autoscore[2]]
#           } else { 
...
#   } 
#   for (i in SC) if (any(is.na(alldat[[i]])))
...
# 
# alldat=green;S="s";R="R";RT="RT";SC="C";F=c("SF","D","WL")
# autoscore=list(C=c(sfac="SF",rfac="R"))
# autoscore.list=list(C=list(non.target=c("non.target"),
#                            target=c("easy.target","hard.target")))

# alldat=dat1;S="s";R="R";RT="RT";F=c("RS","IS");CV="CI"
#   SC=c("C","SR")
#   autoscore=list(C=c(sfac="RS",rfac="R"),SR=list(sfac=c("RS","IS","R"))) 
#   autoscore.list=list(SR=SR)

#   alldat=alldat=dat2;S="s";R="R";RT="RT";F=c("RSS","ISS","M")
#   CV=c("CI");SC=c("C","SR")
#   autoscore=list(C=list(sfac=c("M","RSS","R")),SR=list(sfac=c("RSS","ISS","M","R")))
#   autoscore.list=list(C=C2,SR=SR2)

# alldat=data;S="s";R="R";RT="RT";SC="C";F=c("CR","NS")
...
score.rc <- function(alldat,
  ...
  score <- function(dat,S,SC,CV,RT,default.scores=NA,autoscore=NA,autoscore.list=NA) {
    ...
    for (i in SC) {
      ...
      boolean.score <- length(autoscore[[i]])>1
      for (j in slevs) {
        cat(paste("Scoring subject",j,"\n"))
        Di=out[[j]]
        dati <- dat[dat[[S]]==j,]
        ...
        if ( any(badscore) ) {
          if ( boolean.score )  
            Di$D[,i] <- factor(as.character(Di$D[,i]),levels=c("FALSE","TRUE")) else
            Di$D[,i] <- factor(as.character(Di$D[,i]),levels=okscore)
        ...
        if ( !is.factor(dat[[i]]) ) 
          Di$D[[i]] <- factor(as.character(Di$D[[i]])) else
          Di$D[[i]] <- factor(as.character(Di$D[[i]]),levels=levels(dat[[i]]))
        out[[j]] <- Di
      }
    }
    attr(dat,"D")=out
...
#   minrt=.2;maxrt=2.5;qp=NA;correct.name="C"
#   save.trials=FALSE;fremove=NULL;sremove=NULL;show.summary=TRUE
#   show.censors=FALSE;show.cell.censors=FALSE;show.cell.censor=FALSE

# dat=sh1.df;minrt=.15;maxrt=1
...
make.rc <- function(dat,      # data frame prepared by score.rc()
  ...
  save.trials=FALSE,          # Add in an attribute to identify trials 
...
get.simdat <- function(dname,mname,ss=NA,excludeS=NA) {
  ...
  for (i in ss) {
    ...
    if ( i == ss[1] ) {
      ...
      new.names <- names(sim)
      oldD <- attr(dat,"D")
      dat <- cbind(dat,data.frame(matrix(ncol=length(new.names),
        nrow=dim(dat)[1])))
      names(dat)[-c(1:n.old)] <- new.names
      attr(dat,"D") <- oldD
    }
    dat[dat[[D$S]]==i,new.names] <- sim
  }
...
table.rc <- function(rc,
  ...
  measure = c("accuracy","inaccuracy","probit","rt","correct","error")[1],  
  ...
  if (measure=="inaccuracy") {
    measure <- "accuracy"
    inaccuracy <- TRUE
  } else inaccuracy <- FALSE
...
#   stat = c("mean","quantile","sd","var")[1];factors=NA;probs=.5
#   xlim=NA; ylim=NA;xlab=NA;plot.subjects=FALSE;bars="se"; dat.save=NULL
#   bar.type=c("within","between","data","model","pbmodel")[1]
#   include.subjects=NA;exclude.subjects=NA;exclude.incomplete=FALSE                           
#   plot.model=FALSE;model.name="fit";data.name="data";data.fit.name="Type"                              
#   plot.lines=TRUE;cex=1;save.dat=FALSE;do.plot=T;probit.errors = FALSE
#   layout=NULL; do.response=FALSE 
# 
# rc=lba1;measure="inaccuracy";plot.subjects=FALSE;bars=95
# plot.model=T;model.name="LBA";do.response=TRUE
...
plot.rc <- function(rc,
  ...
  factors=NA,                                         # subset of factor names
  do.response=FALSE,                                  # add in Response factor
  ...
  cex=1,
  save.dat=FALSE,                                     # save av object
  dat.save=NULL,                                      # use saved av
  do.plot=T,
  probit.errors = FALSE,                             # Snodgrass and Cowrin + probit SEs
...
#   outc=fixholes(dat,Di,"accuracy",plot.model)
  ...
  # Get factors
  if ( do.response ) {
    cat("Adding response factor\n")
    rnam <- attributes(rc$dat)$D[[1]]$R
    if (!any(is.na(factors))) 
      factors <- c(rnam,factors) else
      factors <- c(rnam,Di$F)
    if (measure %in% c("accuracy","inaccuracy"))
      measure <- "presp" # probability response
  }
  if (any(is.na(factors))) wsF <- Di$F else wsF <- factors
  ...
  # Get measure
  measures <- c("rt","correct","error","accuracy","inaccuracy","stop","presp")
  if (is.na(pmatch(measure[1],measures)))
  ...
  tmp <- dat[is.finite(dat[,Di$RT]),]
  sfacs <- c(Di$S,Di$F)
  ns <- arr2df(tapply(tmp[,Di$RT],tmp[,sfacs],length)) 
  ...
  dat <- switch(pmatch(measure[1],measures),
    ...
    {ylab <- "Response (%)"; dat <- dat[is.infinite(dat[,Di$RT]),]}
  )  

  probit.errors <- measure[1] %in% c("accuracy","inaccuracy") & probit.errors
  if (probit.errors) {
    dat$qp[do.call(order,dat[,sfacs])] <- qnorm(
      (dat$qp[do.call(order,dat[,sfacs])]*ns$y[do.call(order,ns[,sfacs])]+.5)/
      (ns$y[do.call(order,ns[,sfacs])]+1))
    dat$qp.sim[do.call(order,dat[,sfacs])] <- qnorm( 
      (dat$qp.sim[do.call(order,dat[,sfacs])]*ns$y[do.call(order,ns[,sfacs])]+.5)/
      (ns$y[do.call(order,ns[,sfacs])]+1))
  } 
...
#   bad <- apply(data,sdim,function(x){any(is.na(x))})
  ...
  if (probit.errors) av$y <- pnorm(av$y/100)*100
  ...
  if (!is.null(dat.save)) av <- dat.save
  if (do.plot) {  
...
# layout=NULL
# 
# rc=lba3;measure="correct";probs=c(.1,.5,.9);plot.subjects=FALSE;save.dat=T;do.plot=F
# plot.model=T;model.name="LBA";bars=95;factors=c("S","PM");do.response=TRUE
...
plotqs.rc <- function(rc,
  ...
  # wsF=dwsFs; verbose=F;sim=F;qnam=data.fit.name
...
plotce.rc <- function(rc,
  ...
  # wsF=dwsFs; verbose=F;sim=T;qnam=data.fit.name
...
#   check(wsF,Di$F,data.fit.name)
  ...
  if (any(is.na(xlim)) & any(is.na(ylim))) {
    if (!plot.model) 
      print(xyplot(formula(plotform),av,groups=av[,wsF[1]],xlab=xlab,key=key,panel=panel.se,
    ...
    if (!plot.model) 
      print(xyplot(formula(plotform),av,groups=av[,wsF[1]],xlab=xlab,key=key,panel=panel.se,cex=cex,
    ...
    if (!plot.model) 
      print(xyplot(formula(plotform),av,groups=av[,wsF[1]],xlab=xlab,key=key,pch=pchs,cex=cex,
        type=c("p","l"),ylab=ylab,ylim=ylim,as.table=T,col=cols,panel=panel.se,
             strip = strip.custom(bg="white"))) else 
      print(xyplot(formula(plotform),av,groups=av[,wsF[1]],key=key,
        type=c("p","l"),ylab=ylab,pch=pchs,lty=ltys,col=cols,cex=cex,
...
##############  Trial covariate verison


# tcv=NA; measure = c("accuracy","inaccuracy","rt","correct","error")[1]
# stat = c("mean","quantile","sd","var")[1]
# factors=NA;do.response=FALSE;probs=.5;xlim=NA;ylim=NA;xlab=NA;
# plot.subjects=FALSE;bars="se"
# bar.type=c("within","between","data","model","pbmodel")[1]
# include.subjects=NA;exclude.subjects=NA;exclude.incomplete=FALSE
# plot.model=FALSE;model.name="fit";data.name="data";data.fit.name="Type"
# plot.lines=TRUE;cex=1;save.dat=FALSE;dat.save=NULL;do.plot=T;layout=NULL

# rc=lba;nblock=5;measure="inaccuracy";plot.subjects=FALSE;plot.model=TRUE
# factors=c("T","PA") 

plott.rc <- function(rc,nblock,
  tcv=NA,
  # raw accuracy, all/correct/error rt
  measure = c("accuracy","inaccuracy","rt","correct","error")[1],  
  stat = c("mean","quantile","sd","var")[1], # name of any statistic
  factors=NA,                                         # subset of factor names
  do.response=FALSE,                                  # add in Response factor
  probs=.5,                                           # quantile probability
  ...
  # wsF=dwsFs;verbose=F;sim=F
  getstat <- function(dat,measure,stat,probs,wsF,Di,verbose=F,sim=F) {
    pc <-c("accuracy","inaccuracy","stop","presp")
    if ( sim ) {
      qpnam=paste(Di$SC[1],"sim",sep=".")
      if ( !any(names(dat)==qpnam) )
        stop("Simulation results have not been added to rc object")
      qs <- unlist(lapply(strsplit(names(dat),"-"),function(x){
        if (length(x)>1 & x[[1]]=="q") x[[2]]}))
      rtnam <- paste("q",qs,sep="-")
    } else {
      rtnam <- Di$RT
      qpnam=Di$SC[1]
    }
    if ( stat[1]=="quantile" ) {
      ...
      if ( !is.null(qp) ) {
        ...
        if ( any(is.na(dat[,rtnam])) ) {
#           bad <<- dat; rtnams <<- rtnam          
          stop(paste("NAs present in statistic",rtnam))
        }
        if (length(rtnam) > 1) {
          tmp <- vector(mode="list",length=length(rtnam))
          names(tmp) <- rtnam
          for (i in rtnam) tmp[[i]] <- cbind(dat[,wsF],y=dat[,i])
          dat <- do.call(rbind,tmp)
          rtnam <- paste(Di$RT,"sim",sep=".")
          names(dat)[dim(dat)[2]] <- rtnam
        } 
        data <- tapply(dat[,rtnam],dat[,wsF],stat,probs=probs[1])
      }
    } else {
  ...
  # Main body 
  if (do.response) stop("Response mode not implemented")
  if (nblock==1) stop("Must be more than one block (nblock>1)")
  require(lattice)
  if (bar.type=="pbmodel") {
    bar.type="model"
    pb <- TRUE
  ...
  dat <- switch(pmatch(measure[1],measures),
    {ylab <- "All RT (s):"; dat[is.finite(dat[[Di$RT]]),]},
    {ylab <- "Correct RT (s):"; 
      dat[is.finite(dat[[Di$RT]]) & as.logical(dat[,Di$SC[1]]),]},
    {ylab <- "Error RT (s):"; 
      dat[is.finite(dat[[Di$RT]]) & !as.logical(dat[,Di$SC[1]]),]},
    {ylab <- "Accuracy (%):"; dat[is.finite(dat[,Di$RT]),]},
    {ylab <- "Error Rate (%):"; dat[is.finite(dat[,Di$RT]),]},
    {ylab <- "Response (%)"; dat <- dat[is.infinite(dat[,Di$RT]),]}
  )  
  if (stat!="quantile") ylab <- paste(ylab,stat) else
    ylab <- paste(ylab,probs,stat)  

  # Modify trial covariate to a block covariate
  tmp <- tapply(dat[[tcv]],dat[[Di$S]],function(x){
    as.numeric(cut(x,c(0,quantile(x,probs=c(1:(nblock-1))/nblock),Inf)))})
  for (i in names(tmp)) dat[dat[[Di$S]]==i,tcv] <- tmp[[i]]
