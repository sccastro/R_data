...
sfun <-function(dati,pnams,t0tune=.9,aprior=.5,
  st=.05,sz_on_a=.2,sv_on_v=.2,pc=.01,pg=.5,s=.1) {
...
pfun=function(ts,parlist,doCDF=F,precision=2.5) {
  ...
  parlist[1,c("a","v","sz","sv","z")] <- 
    parlist[1,c("a","v","sz","sv","z")]/parlist[1,"s"]
...
#  if (parlist[1,"a"]>100 | parlist[1,"sz"]>10) return(out)
  ...
  para <- c(parlist$a[1],parlist$v[1],ter,
    parlist$sz[1],parlist$sv[1],parlist$st[1],
    parlist$z[1])
...
#   nmc=10;v=.01;a=1;z=.5;sz=0;sv=0;t0=.1;st=.1;h=.001;s=.1
  ...
  repeat {
    sims <- mc.RD.C(onen,
      v=pi$v/(pi$s*10),a=pi$a/(pi$s*10),z=pi$z/(pi$s*10),sz=pi$sz/(pi$s*10),sv=pi$sv/(pi$s*10),
      t0=pi$t0,st=pi$st)    
    if (remove.outliers) {
