...
#  print(i)     
      ...
      if ( dim(dati)[1]>0 ) {
        pc <- p$pc[isin][1]
        parlist <- p[isin,]
        parlist$ter <- parlist$ter + 
          parlist$shape*as.numeric(as.character(D$D[i,"A"]))/180
        L <- c(L, pc*cp + (1-pc)*
          dfun(t=dati[[D$RT]],parlist=parlist,cv=dati[,D$cv,drop=F]),
  ...
  if ( exists("dfun") ) { # likelihood may not be available for some models
    ...
    for (i in levels(D$D$lcell)) {
      ...
      if ( any(isrt) ) {
        ...
        parlist$ter <- parlist$ter + 
          parlist$shape*as.numeric(as.character(D$D[i,"A"]))/180
        dat$lm[isrt] <- dfun(t=dat[isrt,D$RT],parlist=parlist,cv=dat[isrt,D$cv,drop=F])
...
#     crct <- dat[dat$rcell==rcnams[i] & isrt,cnam]
    ...
    if ( cv!="" ) {
      ...
      tmp$n <- data.frame(tmp$n)
       ...
       parlist <- p[isin,]
       parlist$ter <- parlist$ter + 
          parlist$shape*as.numeric(as.character(D$A[isin]))/180
      tmp <- rfun(ns=nsim,pi=parlist,tlohi=tlohi,maxsamp=maxsamp,sim.mult=sim.mult)
    }
    sim[[i]] <- tmp$sims
