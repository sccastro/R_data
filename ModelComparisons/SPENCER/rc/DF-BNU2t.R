# "truncdrifts" version, pie slice from MVN drifts
...
# silent=F; A=c(1,1); b=c(2,2); vC=c(2,1); vE=c(1,2); sv=c(.1,.1); ter=c(.1,.1); st0=c(.1,.1)
# parlistC=list(A=A,b=b,v=vC,sv=sv,ter=ter)
# parlistE=list(A=A,b=b,v=vE,sv=sv,ter=ter)
# parlistCst=list(A=A,b=b,v=vC,sv=sv,ter=ter,st0=st0)
# parlistEst=list(A=A,b=b,v=vE,sv=sv,ter=ter,st0=st0)
# x=c(1:300)/100; par(mfcol=c(1,2))
# plot(x,dfun(t=x,parlistC,silent=silent),type="l");abline(h=0); lines(x,dfun(t=x,parlistCst,silent=silent),col="red")
# plot(x,dfun(t=x,parlistE),type="l");abline(h=0); lines(x,dfun(t=x,parlistEst,silent=silent),col="red")
# pfun(Inf,parlistC,silent=silent)+pfun(Inf,parlistE,silent=silent)
# pfun(Inf,parlistCst,silent=silent)+pfun(Inf,parlistEst,silent=silent)
...
dfun=function(t,parlist,silent=TRUE,cv=NULL) {  
  ...
  fptpdf=function(z,A,b,v,sv) {
    ...
    if (A<1e-10) # LATER solution 
      return( (b/z^2)*dnorm(b/z,mean=v,sd=sv) )
    zs=z*sv ; zu=z*v ; bminuszu=b-zu
    bzu=bminuszu/zs ; bzumax=(bminuszu-A)/zs
    (v*(pnorm(bzu)-pnorm(bzumax)) +
      sv*(dnorm(bzumax)-dnorm(bzu)))/A
  }

  fptcdf=function(z,A,b,v,sv) {
    # cdf for a single unit
    if (A<1e-10) # LATER solution 
      return( pnorm(b/z,mean=v,sd=sv,lower.tail=F) )
    zs=z*sv ; zu=z*v ; bminuszu=b-zu ; xx=bminuszu-A
    bzu=bminuszu/zs ; bzumax=xx/zs
    tmp1=zs*(dnorm(bzumax)-dnorm(bzu))
    tmp2=xx*pnorm(bzumax)-bminuszu*pnorm(bzu)
    1+(tmp1+tmp2)/A
  }
...
#   A=parlist$A;b=parlist$b;v=parlist$v;sv=parlist$sv
  n1PDFfixedt0=function(t,A,b,v,sv,silent=T) {
    ...
    out <- out/(1-prod(pnorm(-v/sv))) # truncdrifts
  ...
  if (!any(names(parlist)=="st0") || parlist$st0[1]<.001) { 
    ...
    for (i in 1:length(outs)) {
      lower=t[i]-parlist$st0[1]
      upper=t[i]
      if (any(c(lower,upper)<0)) 
        tmp <- 0 else 
        tmp <- try(integrate(f=tmpf,lower=lower,upper=upper,
          A=parlist$A,b=parlist$b,v=parlist$v,sv=parlist$sv,
...
#   n: number samples for each accumulator
  ...
  repeat {
    ...
    # remove ALL neg v cases
    rts <- rts[,apply(rts,2,function(x){any(x>0)}),drop=F]     
...
# silent=F; A=c(1,1); b=c(2,2); vC=c(2,1); vE=c(1,2); sv=c(3,3); ter=c(.1,.1); st0=c(.1,.1)
# parlistC=list(A=A,b=b,v=vC,sv=sv,ter=ter)
# parlistE=list(A=A,b=b,v=vE,sv=sv,ter=ter)
# parlistCst=list(A=A,b=b,v=vC,sv=sv,ter=ter,st0=st0)
# parlistEst=list(A=A,b=b,v=vE,sv=sv,ter=ter,st0=st0)
...
# print(pfun(Inf,parlistCst,silent=silent)+pfun(Inf,parlistEst))
