# Orignal version drifts can be negative, ONLY DIFFERENT FROM NON-vcve VERSION IN THAT
# M2P PASSES Di in sfun
...
#  A = b*Atune
  ...
  M2P(data.frame(LBAstarts),Di)
}


dfun=function(t,parlist,cv=NULL,precision=NA) {  
  ...
  if (!any(names(parlist)=="st0") || parlist$st0[1]<.001) { 
    ...
    for (i in 1:length(outs)) {
      tmp <- try(integrate(f=tmpf,
        ...
        A=parlist$A,b=parlist$b,v=parlist$v,sv=parlist$sv,
        st0=parlist$st0[1])$value,silent=T)
      if (is.numeric(tmp)) 
        outs[i] <- tmp else
        outs[i] <- 0
    }
...
pfun=function(t,parlist,doCDF=T,cv=NULL,precision=NA) {
...
# ns=nsim;pi=p[isin,]
# cv=NULL; remove.outliers=TRUE; onen=1e4; warn=TRUE; maxsamp=1e6
...
# Parameters show 10% low through neither positive
# A=c(1,1); b=c(2,2); vC=c(2,.1); vE=c(.1,2); svC=c(3,1); svE=c(1,3); ter=c(.1,.1); st0=c(.1,.1)
