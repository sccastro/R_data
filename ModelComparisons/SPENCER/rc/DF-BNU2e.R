...
dfun=function(t,parlist,cv=NULL) {  
  ...
  if ( !any(names(parlist)=="st0") || parlist$st0[1]<.001 ) { 
    ...
    tmpf <- function(tau,ti0,A,b,v,sv,rate) 
      n1PDFfixedt0(tau,A,b,v,sv)*dexp(ti0-tau,rate)
    outs <- numeric(length(t))
    for (i in 1:length(outs)) {
      tmp <- try(integrate(f=tmpf,
        lower=0,upper=t[i],ti0=t[i],
        A=parlist$A,b=parlist$b,v=parlist$v,sv=parlist$sv,
        rate=1/parlist$st0[1])$value,silent=T)
      if (is.numeric(tmp)) 
        outs[i] <- tmp else
        outs[i] <- 0
    }
...
#   n: number samples for each accumulator
  ...
  repeat {
    ...
    if ( Ni>0 ) {
      ...
      # t0 noise
      if ( any(names(pi)=="st0") ) 
        win[2,] <- win[2,] + rexp(Ni,1/pi$st0[1])
      if ( remove.outliers ) {
